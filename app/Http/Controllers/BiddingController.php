...
class BiddingController extends Controller
{
    ...
    public function save_interest(Request $request){
                ...
                //from freelancer to client
                NotificationUtility::set_notification(
                    "Tradesmen_Interest_on_job",
                    translate('A New Interest has been submitted by'),
                    route('project.details',['slug'=>$project->slug],false),
                    $project->client_user_id,
                    Auth::user()->id,
                    'client'
                ...
                EmailUtility::send_email(
                    translate('A new Interest has been submitted'),
                    translate('A new Interest has been submitted by'). Auth::user()->name,
                    get_email_by_user_id($project->client_user_id),
                    route('project.details',['slug'=>$project->slug])
                );
    
                flash(translate('Your Interest for this job has been submitted successfully'))->success();
                return redirect()->back();
    }
}
